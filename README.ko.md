...
AI-Core는 앱 서버가 보낸 현재 상태 복사본(snapshot)을 받아 검색용 데이터, 문서 index
record와 signal, 가벼운 폴더 metadata 인덱스, 그래프 관계, 워크플로우 상태 같은 파생
데이터만 만듭니다.
...
| 영역 | 하는 일 |
| --- | --- |
| 문서 인덱싱 | 문서 본문을 작은 조각(chunk)으로 나누고, 문서 index record와 signal을 만든 뒤 비동기 vector/graph projection용 outbox event를 발행합니다. |
| 폴더 인덱싱 | 가벼운 폴더 index 상태를 저장하고 비동기 folder vector/graph hierarchy projection용 outbox event를 발행합니다. |
...
| 저장 범위 | 원본 데이터가 아니라 문서 index record, document signal, vector payload, graph 관계, task snapshot, task event 같은 파생 데이터만 저장합니다. |
...
| AI-Core | 문서 index record, document signal, vector index payload, graph DB 관계, 검색 결과, 생성 결과, workflow 상태, 제안된 host action |
...
인덱싱 요청은 PostgreSQL에 index record, signal, outbox event를 원자적으로 저장합니다.
Debezium Kafka consumer가 비동기로 embedding을 만들고 Qdrant/Graph DB projection을
수행합니다. Outbox event는 database-generated sequence, idempotency key, generated
`partition_key`를 사용합니다. Delete event idempotency key에는 현재 `source_version`을
포함해 재인덱싱 후 다시 삭제되는 이벤트가 중복으로 삼켜지지 않게 합니다. Qdrant에는 chunk, signal, folder, 문서 단위 vector가 저장됩니다. Graph
DB에는 문서-폴더, 문서-signal, 폴더-signal, 폴더 계층 관계가 저장됩니다. Neo4j
projection 상태는 PostgreSQL에 ledger로 중복 저장하지 않고, outbox replay와 Kafka
dead-letter 처리로 복구합니다. Chunk 본문은 Qdrant에 저장하고 graph DB에는 chunk node를
만들지 않습니다.
...
폴더 추천은 workflow recommendation step으로 실행합니다. AI-Core는 후보 `folder_id`,
score, reason을 반환하고, 원본 데이터 변경 적용은 앱 서버가 담당합니다.
...
| AI-Core가 signal 생성 | LLM으로 summary, concept, entity, issue, commitment, claim signal을 생성합니다. |
| AI-Core가 저장 | PostgreSQL에 index record와 transactional outbox event를 저장합니다. |
| Projection worker 실행 | Outbox consumer가 embedding을 만들고 Qdrant와 graph DB record를 projection합니다. |
...
| 단계 | 일어나는 일 |
| --- | --- |
| 앱 서버가 보냄 | `POST /tasks`에 자연어 질문, tenant, 선택적 context를 보냅니다. |
...
| AI-Core가 답변 생성 | context generation agent가 검색 context를 근거로 답변을 작성합니다. |
| 앱 서버가 받음 | 답변 text와 citation이 담긴 task snapshot을 받습니다. |

### 3. 워크플로우 task 실행

| 단계 | 일어나는 일 |
| --- | --- |
| 앱 서버가 시작 | `POST /tasks`에 tenant와 자연어 요청을 보냅니다. task ID와 첫 task input ID는 AI-Core가 생성합니다. |
| 앱 서버가 이어쓰기 | `POST /tasks/{task_id}/inputs`로 같은 task에 자연어 input을 추가합니다. |
...
| `DELETE` | `/indexing/documents/{document_id}` | - | `204 No Content` | 해당 문서의 index record, signal, vector payload, graph DB 관계를 삭제합니다. |
...
| Method | Path | Request DTO | Response DTO | 목적 |
| --- | --- | --- | --- | --- |
| `POST` | `/tasks` | `CreateTaskRequest` | `TaskSnapshotResponse` | 자연어 input으로 task를 시작합니다. |
| `POST` | `/tasks/{task_id}/inputs` | `AppendTaskInputRequest` | `TaskSnapshotResponse` | 기존 task에 자연어 input을 추가하고 다시 계획합니다. |
| `GET` | `/tasks/{task_id}` | - | `TaskSnapshotResponse` | 앱 서버가 볼 수 있는 최신 task snapshot을 조회합니다. |
| `DELETE` | `/tasks/inputs/{task_input_id}` | - | `TaskSnapshotResponse` | task 안의 input entry를 removed로 표시하고 남은 active input으로 다시 계획합니다. |
...
1. 앱 서버가 `POST /tasks`에 tenant와 자연어 input을 보냅니다. AI-Core가 `task_id`와 `task_input_id`를 생성합니다.
2. 이후 `POST /tasks/{task_id}/inputs`를 호출하면 기존 task에 input을 추가합니다.
3. `TaskWorkflowService`가 현재 `TaskSnapshot`을 저장하고 workflow runtime을 호출합니다.
...
7. step 결과는 `WorkflowArtifactRegistry`를 통해 기록되고, 앱 서버가 볼 수 있는 `TaskFinalResult`로 노출됩니다.
8. 앱 서버는 `DELETE /tasks/inputs/{task_input_id}`로 특정 input entry를 removed 처리할 수 있습니다.
...
| `DocumentSignalExtractorAgent` | `document_signal_extraction` | `DocumentIndexingService` | 문서와 chunk를 분석해 `DocumentIndexRecord`와 `DocumentSignal` 집합을 생성합니다. |
...
| Signal | `DocumentSignal`, `FolderSignal` | signalized summary, concept, entity, issue, commitment, claim 결과입니다. |
...
| Workflow | `TaskInputEntry`, `TaskSnapshot`, `TaskAnalysis`, `TaskFinalResult`, `TaskEvent` | 앱 서버가 조회하는 task input 이력, 상태, output입니다. |
| Host action | `HostAction`, `HostActionResult`, action input/output payload | 앱 서버가 실행해야 하는 제안된 원본 데이터 변경입니다. |
| Knowledge graph | `DocumentRelationshipProjection`, `DocumentSignalGraphProjection`, `FolderRelationshipProjection` | 문서, 폴더, document/folder signal 사이의 파생 관계입니다. |
| Outbox event | `OutboxEvent` | PostgreSQL index record와 signal 변경과 같은 transaction에서 발행되는 projection 입력 event입니다. |

중요한 규칙:

- source model은 snapshot입니다. 원본 데이터의 기준은 앱 서버입니다.
- indexed model, index record, signal은 다시 만들 수 있는 파생 상태입니다.
- `TaskFinalResult`는 type이 정해져 있습니다. 예를 들어 `summary` result는 generated text를 담아야 합니다.
...
| `document_sources`, `folder_sources` | Source IDs | 현재 source manifest. source identity, aggregate source version, digest/size 또는 folder metadata, timestamp, 삭제 상태, opaque source metadata를 저장하며 원문 body는 저장하지 않음 |
| `source_document_folder_relations` | `(tenant_id, document_id, folder_id)` | document relation snapshot에서 온 현재 folder membership row. row가 없으면 현재 empty membership |
| `document_index_records`, `document_chunks`, `folder_index_records` | Source IDs / chunk UUIDs | 현재 파생 index manifest와 document chunk record |
| `document_signals`, `folder_signals` | `signal_id` | 추출된 signal text, payload, evidence, confidence, extractor metadata, source input digest, generation version, 선택적 generation model |
| `vector_projection_records` | `(collection_name, point_id)` | Qdrant write ledger. Vector record는 source row FK 없이 collection, point ID, source identity, vector item identity, source input digest, vector input digest를 기록 |
| `outbox_events` | `event_id` UUID | Kafka/Debezium transactional outbox event. `tenant_id`, `idempotency_key`, `event_sequence`, `partition_key`, `event_type`으로 stream ordering, idempotency, dead-letter context 제공 |
| `tasks` | `task_id` UUID | 현재 task aggregate status, active input text, analysis message, current action, error, metadata |
| `task_inputs` | `task_input_id` UUID | task 안에 쌓인 사용자 요청 entry와 active/removed 상태 |
| `task_jobs`, `task_job_results` | UUID job/result IDs | 계획된 workflow job, 실행 상태, typed result payload, task-local ordering |
...
| `FOLDMIND_QDRANT_SIGNAL_COLLECTION` | `signals` | `signal` | 추출된 document/folder signal에 대한 signal 단위 vector search |
...
| `Document` - `Signal` | 문서 버전에 어떤 추출된 signal이 연결되는지 |
...
| `core/application/ports/inbound/` | inbound adapter가 호출하는 protocol입니다. Concrete application service가 구조적으로 구현합니다. |
...
| `core/application/models/` | application service command, query, result, retrieval model, projection model, workflow flow-state model입니다. |
| `core/application/mappers/` | boundary mapping과 domain-to-application mapping 함수입니다. |
| `core/application/services/` | indexing, retrieval, recommendation, projection, workflow 책임별 inbound application API와 application policy입니다. |
| `core/application/formatters/`, `execution/`, `prompts.py` | service가 아닌 application support code입니다. |
...
| `bootstrap/` | 설정을 읽고 concrete adapter와 application service를 port 뒤에 연결합니다. |
...
`FOLDMIND_DOCUMENT_SIGNAL_EXTRACTION_PROMPT_VERSION`은 표준 configured API에서 필수입니다.
...
기본 outbox topic은 `indexing-events`이며, Debezium Kafka message key는 generated
`partition_key` column으로 설정합니다. 실패 메시지는 `indexing-events.dlq`로 보낸 뒤
원인 수정 후 `scripts/replay_dead_letter_events.py`로 재생합니다.
