...
앱 서버는 원본 문서, 폴더, 태그, 권한, 사용자, 비즈니스 규칙을 계속 소유합니다.
...
| 소유자 | 소유 대상 |
| --- | --- |
| FoldMind 앱 서버 | 원본 문서, 원본 폴더, 태그, 권한, 사용자, 비즈니스 규칙 |
...
문서 AI 프로필(`DocumentProfile`)을 생성합니다. 이 프로필은 원본 title snapshot,
요약, 파생 concept, profile confidence, 사용한 모델과 prompt 버전을 담습니다.

인덱싱 요청은 PostgreSQL에 프로필과 outbox event를 원자적으로 저장합니다.
Qdrant/Graph DB projection은 Debezium Kafka consumer가 비동기로 만듭니다. Qdrant에는 chunk 검색용 vector와 문서 단위 검색용 vector가 저장됩니다. Graph DB에는
문서-폴더, 문서-태그, 문서-concept, 태그-concept, 폴더 계층 관계가 저장됩니다.
...
2. Graph DB는 같은 폴더, 태그, 파생 concept로 연결된 문서를 찾습니다.
3. AI-Core는 두 결과에서 문서 ID를 모으고, 그 문서들 안에서 다시 관련 chunk를 찾습니다.
4. 검색된 chunk를 점수순으로 정렬하고, relevance validation agent가 질문과 무관한
   chunk를 걸러냅니다.
...
| 단계 | 일어나는 일 |
| --- | --- |
| 앱 서버가 보냄 | `POST /indexing/documents`에 문서 제목, 본문, 버전, 폴더 ID, 태그, metadata를 보냅니다. |
...
| AI-Core가 프로필 생성 | LLM으로 title 기반 요약, 파생 concept, profile confidence를 생성합니다. |
...
| AI-Core가 chunk 검증 | relevance validation agent가 질문과 무관한 chunk를 제거합니다. |
...
| `POST` | `/indexing/documents/delete` | `DeleteDocumentIndexRequest` | `DeleteDocumentIndexResponse` | 해당 문서의 profile, vector payload, graph DB 관계를 삭제합니다. |
| `POST` | `/indexing/folders` | `IndexFolderRequest` | `IndexFolderResponse` | 폴더 snapshot을 인덱싱하고 검색용 folder 모델을 반환합니다. |
| `POST` | `/indexing/folders/delete` | `DeleteFolderIndexRequest` | `DeleteFolderIndexResponse` | 해당 폴더의 vector payload와 graph DB 관계를 삭제합니다. |
...
| `GET` | `/tasks/{tenant}/{task_id}` | - | `TaskSnapshotResponse` | 앱 서버가 볼 수 있는 최신 task snapshot을 조회합니다. |
| `DELETE` | `/tasks/{tenant}/{task_id}/requests/{task_request_id}` | - | `TaskSnapshotResponse` | task 안의 요청 entry를 removed로 표시하고 남은 active 요청으로 다시 계획합니다. |
...
3. Graph DB에서 질문 text와 연결된 태그, 폴더, 파생 concept를 통해 관련 문서 ID를 찾습니다.
4. 2번과 3번에서 문서 ID가 나오면 그 문서들 안에서 chunk vector 검색을 실행합니다.
5. 문서 ID가 나오지 않으면 요청의 `SearchScope`만 적용해 chunk vector 검색을 실행합니다.
6. optional keyword repository가 설정되어 있고 `hybrid` mode이면 dense 결과와 keyword 결과를 reciprocal rank fusion으로 합칩니다.
7. `ChunkRelevanceValidatorAgent`가 질문과 관련 없는 chunk를 제거합니다.

현재 Qdrant adapter는 dense chunk vector, document-level vector, folder vector를
구현합니다. keyword search는 optional `DocumentKeywordRepository`로 분리되어 있고,
현재 Qdrant adapter는 keyword indexing을 구현하지 않습니다.
...
6. `WorkflowStepExecutor`가 각 step을 검색, 추천, 생성, host-action planning handler로 보냅니다.
7. step 결과는 `WorkflowArtifactStore`에 저장되고, 앱 서버가 볼 수 있는 `TaskOutput`으로 노출됩니다.
8. 앱 서버는 `DELETE /tasks/{tenant}/{task_id}/requests/{task_request_id}`로 특정 요청 entry를 removed 처리할 수 있습니다.
...
agent는 OpenAI SDK에 직접 의존하지 않고 `LLM`과 `PromptRepositoryPort`에 의존합니다.

| Agent | Prompt key | 사용 위치 | 책임 |
| --- | --- | --- | --- |
| `PlanningAgent` | `workflow_planning` | `WorkflowEngine.prepare()` | 자연어 task를 검증된 `WorkflowPlan`으로 바꿉니다. |
| `DocumentProfilerAgent` | `document_profiling` | `IndexDocumentUseCase` | 문서와 chunk를 분석해 `DocumentProfile`을 생성합니다. |
| `ChunkRelevanceValidatorAgent` | `chunk_relevance_validation` | `FindDocumentsUseCase` | 검색된 chunk 중 질문과 관련 있는 chunk만 남깁니다. |
| `AnswerGeneratorAgent` | `answer_generation` | `AnswerQuestionUseCase`, workflow `answer_question` step | 검색된 chunk를 근거로 답변을 생성하고 근거를 함께 반환합니다. |
| `SummarizerAgent` | `summarization` | workflow summary/report step | 검색된 chunk를 근거로 요약문을 생성합니다. |
| `DraftGeneratorAgent` | `draft_generation` | workflow `generate_draft` step | 사용자 지시와 검색된 chunk 본문을 바탕으로 초안을 생성합니다. |
| `IdeasExplorerAgent` | `ideas_exploration` | workflow `explore_ideas` step | 사용자 prompt와 검색된 chunk 본문을 바탕으로 아이디어를 생성합니다. |

RAG 계열 agent는 검색된 chunk를 LLM에 전달하기 전에
...
| AI profile | `DocumentProfile` | LLM이 만든 문서 title snapshot, 요약, 파생 concept, profile confidence입니다. |
| Retrieval | `AIQuery`, `RequestContext`, `SearchScope`, `QueryAnchor` | 질문 text, tenant 정보, 검색 범위, anchor입니다. |
...
| Knowledge graph | `DocumentRelationshipProjection`, `DocumentConceptProjection`, `FolderRelationshipProjection`, `TagProjection` | 문서, 폴더, 태그, concept 사이의 파생 관계입니다. |
...
- DTO가 API 입력을 검증한 뒤 plain domain state container로 매핑합니다.
...
| --- | --- | --- |
| `document_profiles` | `document_id` | 문서 프로필의 최신 title, summary, concepts JSON, profile confidence, source/profile/schema version, metadata |
| `outbox_events` | `id` UUID | Debezium Kafka consumer가 Qdrant와 graph DB projection을 만들 수 있도록 transaction 안에서 발행되는 outbox event |
...
| `host_actions` | `action_id` UUID | 제안된 host action, typed input payload, result payload, policy JSON, task-local ordering |
| `host_action_dependencies` | `action_id` + `depends_on_action_id` | FK로 검증되는 host action dependency edge |
...
초기 migration chain은 빈 DB에 정규화된 profile, index, task table을 생성합니다.
기존 저장 형식에서 전환하는 단계는 포함하지 않습니다.

### Qdrant

| Setting | 기본 Collection | Payload Kind | 목적 |
| --- | --- | --- | --- |
| `QDRANT_DOCUMENT_CHUNK_COLLECTION` | `document_chunks` | `document_chunk` | chunk 본문에 대한 vector search |
| `QDRANT_DOCUMENT_COLLECTION` | `documents` | `document` | 문서 프로필을 포함한 문서 단위 vector search |
| `QDRANT_FOLDER_COLLECTION` | `folders` | `folder` | 폴더 discovery와 추천을 위한 folder metadata vector search |
...
| `Document` - `Tag` | 문서가 어떤 태그를 갖는지 |
| `Document` - `Concept` | 문서 프로필이 어떤 파생 concept를 다루는지 |
| `Tag` - `Concept` | tag가 어떤 파생 concept를 대표하는지 |
...
`POSTGRES_DSN`을 통해 PostgreSQL에 저장합니다. in-memory checkpointer는 local/test
설정에서만 사용합니다.
...
| Package | 역할 |
| --- | --- |
| `domain/` | business model과 validation입니다. 외부 framework에 의존하지 않습니다. |
| `application/ports/inbound/` | 앱 서버가 호출하는 use case contract입니다. |
| `application/ports/outbound/` | LLM, embedding, repository, vector index, graph DB, prompt, workflow runtime interface입니다. |
| `application/use_cases/` | indexing, retrieval, recommendation, workflow task orchestration입니다. |
| `application/agents/` | prompt-backed AI 작업입니다. |
| `application/workflows/` | workflow step 실행, artifact 저장, host action 처리 정책입니다. |
| `adapters/inbound/http/` | FastAPI router, REST DTO, HTTP error mapping입니다. |
| `adapters/outbound/` | PostgreSQL, Qdrant, graph DB, OpenAI, LangGraph, file prompt 구현입니다. |
...
`PROFILE_VERSION`, `PROFILE_SCHEMA_VERSION`, `DOCUMENT_PROFILE_PROMPT_VERSION`은
표준 configured API에서 필수입니다.

예시 환경 파일:

- `.env.example`: 지원하는 설정을 나열한 루트 템플릿입니다.
- `examples/env/local.env`: local self-hosted endpoint 예시입니다.
- `examples/env/hybrid.env`: local PostgreSQL과 외부 Qdrant/graph DB 조합 예시입니다.
...
foldmind-ai-core/
  resources/
    prompts/       로컬 편집/참고용 prompt template source copy
  scripts/         로컬 helper entrypoint
  src/foldmind_ai_core/
    ...
    domain/        business model과 validation
    application/   use case, port, agent, workflow policy
    adapters/      inbound/outbound concrete adapter
    shared/        공유 primitive type과 validation utility
  tests/
    unit/          활성 unit test
    contract/      활성 app-server/schema contract test
    integration/   향후 외부 서비스 통합 테스트용 placeholder package
    e2e/           향후 end-to-end flow 테스트용 placeholder package
```

패키지 이름은 `foldmind_ai_core`입니다. 이전 `ai_core` 패키지 경로는 제거되었습니다.
...
Kafka outbox projection worker를 실행합니다. `scripts/reindex_documents.py`는
standalone reindex entrypoint가 wiring되기 전까지 placeholder입니다.

outbox worker는 `OUTBOX_PROJECTION_TARGET`별로 하나씩 실행합니다:
`qdrant-document-chunks`, `qdrant-documents`, `qdrant-folders`, `neo4j-graph`.
...
그래도 `POSTGRES_DSN`, `QDRANT_URL`, graph DB 설정을 요구합니다.

## 현재 한계

- Dockerfile과 Compose manifest가 아직 없습니다.
- 현재 Qdrant adapter는 keyword indexing을 구현하지 않습니다.
- `tests/integration/`과 `tests/e2e/`는 placeholder package입니다.
- `scripts/reindex_documents.py`는 placeholder입니다.
