...
class DocumentRetrievalPolicy:
    ...
    def rank_document_candidates(
        ...
        *,
        ...
        candidate_keys = tuple(
            dict.fromkeys((*document_candidates.keys(), *graph_candidates.keys()))
        )
        ...
        for key in candidate_keys:
            ...
            if document_candidate is not None and graph_candidate is not None:
                score = (
                    max(document_candidate.score, graph_candidate.score)
                    + self.config.both_sources_bonus
                )
                candidates.append(
                    DocumentRetrievalResult(
        ...
        for results in (dense_results, keyword_results):
            for rank, result in enumerate(results, start=1):
                ...
                scores_by_id[chunk_id] = scores_by_id.get(chunk_id, 0.0) + (
                    1.0 / (self.config.hybrid_rrf_k + rank)
                )
    ...
    def dedupe_results_by_document(
        self,
        results: list[RetrievalResult],
    ) -> list[RetrievalResult]:
        seen: set[str] = set()
