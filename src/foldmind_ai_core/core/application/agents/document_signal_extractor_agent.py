...
from foldmind_ai_core.core.application.prompts import PROMPT_DOCUMENT_SIGNAL_EXTRACTION
...
class DocumentSignalExtractorAgent:
    llm: LLMProvider
    prompt_store: PromptStore
    prompt_version: str
    model: str
    signal_service: DocumentSignalService = field(default_factory=DocumentSignalService)
    ...
    async def extract(
        ...
        raw = await self._generate(document, chunks)
        try:
            parsed = parse_json_object_output(raw)
        except ValueError:
            raise InvalidAgentOutputError(
                "Document signal extractor response must be a JSON object."
            ) from None

        document_index_input_digest = _document_index_input_digest(chunks)
        document_signal_input_digest = self.signal_service.input_digest(
    ...
    async def _generate(self, document: SourceDocument, chunks: list[DocumentChunk]) -> str:
        system = self.prompt_store.get(PROMPT_DOCUMENT_SIGNAL_EXTRACTION)
        chunk_payload = [
            {
    ...
    def _signals(
        ...
        *,
        ...
        if not isinstance(values, list | tuple):
            raise InvalidAgentOutputError("Document signal extractor response must include signals.")
        if not values:
            raise InvalidAgentOutputError(
                "Document signal extractor response must include at least one signal."
            )
        ...
        summary_count = sum(
            1 for signal in signals if signal.signal_type == DocumentSignalType.SUMMARY
        )
        if summary_count != 1:
            raise InvalidAgentOutputError(
                "Document signal extractor response must include exactly one summary signal."
            )
        signal_ids = [signal.signal_id for signal in signals]
        if len(signal_ids) != len(set(signal_ids)):
            raise InvalidAgentOutputError(
                "Document signal extractor response must not include duplicate signals."
            )
        return signals
    ...
    def _signal(
        ...
        *,
        ...
        return self.signal_service.create(
    ...
    def _signal_type(self, payload: Mapping[str, object]) -> DocumentSignalType:
        ...
        try:
            return self.signal_service.signal_type(value)
    ...
    def _evidence(
        ...
        if not isinstance(values, list | tuple) or not values:
            raise InvalidAgentOutputError(
                "Document signal extractor signals must include non-empty evidence."
            )
        evidence: list[DocumentSignalEvidence] = []
        for item in values:
            ...
            if signal_evidence.chunk_id not in chunk_ids:
                raise InvalidAgentOutputError(
                    "Document signal extractor signal evidence chunk_id must refer to a provided chunk."
                )
            evidence.append(signal_evidence)
        return tuple(evidence)

    def _signal_evidence(self, payload: Mapping[str, object]) -> DocumentSignalEvidence:
        ...
        if start_offset is not None and end_offset is not None and end_offset < start_offset:
            raise InvalidAgentOutputError(
                "Document signal extractor evidence end_offset must be greater than or equal "
                "to start_offset."
            )
    ...
    def _required_text(self, payload: Mapping[str, object], name: str) -> str:
        ...
        if not isinstance(value, str) or not value.strip():
            raise InvalidAgentOutputError(f"Document signal extractor response must include {name}.")
        return value.strip()
    ...
    def _required_confidence(
        ...
        if isinstance(value, bool) or not isinstance(value, int | float):
            raise InvalidAgentOutputError(f"Document signal extractor {name} must be numeric.")
        confidence = float(value)
        if not math.isfinite(confidence) or confidence < 0.0 or confidence > 1.0:
            raise InvalidAgentOutputError(f"Document signal extractor {name} must be between 0 and 1.")
        return confidence
    ...
    def _optional_offset(self, value: object, name: str) -> int | None:
        ...
        if isinstance(value, bool) or not isinstance(value, int) or value < 0:
            raise InvalidAgentOutputError(
                f"Document signal extractor evidence {name} must be a non-negative integer."
            )
        return value

    def _json_object(self, value: object, name: str) -> JsonObject:
        if not isinstance(value, Mapping):
            raise InvalidAgentOutputError(f"Document signal extractor {name} must be a JSON object.")
...
def _document_index_input_digest(chunks: list[DocumentChunk]) -> str:
    if not chunks:
        raise InvalidAgentOutputError("Document signal extractor requires at least one chunk.")
