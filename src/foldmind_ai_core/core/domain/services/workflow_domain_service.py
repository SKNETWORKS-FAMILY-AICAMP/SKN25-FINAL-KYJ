...
class WorkflowDomainService:
    def mark_result(self, snapshot: TaskSnapshot) -> None:
        ...
        for action in snapshot.host_actions:
            if self.is_completed_host_action_status(action.status):
                continue
    ...
    def mark_failed(self, snapshot: TaskSnapshot, exc: Exception) -> None:
        snapshot.status = TaskStatus.FAILED
    ...
    def validate_host_action_result(
        self,
        action: HostAction,
        ...
        if result.output is not None and result.outcome != HostActionResultType.SUCCEEDED:
            raise InvalidInputError(
                "Only succeeded host action results may include output."
            )
        allowed_outcomes = self._allowed_outcomes_for_status(action.status)
    ...
    def is_host_action_attempt_result(self, result: HostActionResult) -> bool:
    ...
    def is_host_action_retry_result(self, result: HostActionResult | None) -> bool:
        return result is not None and result.outcome in {
            HostActionResultType.FAILED,
            HostActionResultType.RETRY,
        }

    def is_completed_host_action_status(self, status: HostActionStatus) -> bool:
        return status in {
    ...
    def host_action_status_for_result(
        ...
        if (
            ...
            and self.can_retry_host_action(action, result)
        ):
            return HostActionStatus.READY
    ...
    def is_pending_host_action(self, action: HostAction) -> bool:
        return action.status == HostActionStatus.READY or (
            action.status == HostActionStatus.PROPOSED
            and action.policy.requires_confirmation
        )
