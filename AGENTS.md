...
- `domain`: 핵심 비즈니스 개념과 규칙
- `application`: 유스케이스와 애플리케이션 흐름
- `ports`: application 계층이 필요로 하는 인터페이스
...
- Document / Folder / Tag / Concept 관계 graph
- folder hierarchy
- graph 기반 탐색과 추천 보강

각 저장소는 자기 책임 외의 데이터를 소유하지 않는다. 예를 들어 folder/tag 관계의 단일 소유자는 Neo4j이며, Qdrant payload에 folder/tag 관계를 중복 저장하지 않는다.
...
이런 로직은 `application/services`로 분리한다.
...
- `RelationshipScopeResolver`: folder/tag scope를 document search scope로 변환
...
- `DocumentChunker`: chunk 생성 정책과 chunk metadata 부여
...
- Agent는 LLM port와 prompt repository port에 의존한다.
