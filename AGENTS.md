...
문서별 folder relation은 별도 state table 없이 membership row로만 저장한다. 해당 문서의 relation row가 0개이면 어떤 폴더에도 속하지 않는 최신 empty membership으로 해석한다. AI-Core는 relation update의 `source_version`이 현재 `document_sources.source_version`과 정확히 같을 때만 membership row를 교체한다.

LLM 모델명은 index record의 freshness key가 아니다. 실제 생성물 provenance가 필요한 경우 `document_signals.generation_model` 또는 `folder_signals.generation_model`처럼 signal row에 둔다. Deterministic extractor 결과는 `generation_model`이 없을 수 있다.
...
- document/folder source snapshot
...
HTTP router, messaging consumer, worker entrypoint는 inbound port protocol에만 의존한다. Concrete application service는 bootstrap composition root에서만 조립한다.
...
- Application Service 안에서 `require_non_blank`, `require_uuid` 같은 boundary validation을 반복하지 않는다.
- 단, repository 조회 결과 없음, workflow task 없음, 후보 결과 없음 같은 application 상태는 Application Service에서 명시적으로 처리할 수 있다.
...
- `SourceDocument`와 `DocumentIndexRecord`에서 chunk와 document vector projection을 만든다.
- `DocumentSignal`에서 document signal graph projection을 만든다.
...
Application service는 inbound adapter가 호출하는 application entrypoint이며, 반복되거나 비대해지는 application 정책을 담는다.
